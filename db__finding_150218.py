...
def WISE(req, RA, DEC):
    ...
    for x in range(0, len(retCutLines)-1):
            ...
            w3.append(retCutData[16])
            w3sig.append(retCutData[17])
            w4.append(retCutData[19])
            w4sig.append(retCutData[20])

    if(checkNull(w1) == 0) or (checkNull(w2) == 0): #there were no w1 or w2 values for this object
        return

    makeCSV(mjd, w1, w1sig, w2, w2sig, w3, w3sig, w4, w4sig, req)
...
def makeCSV(data1, data2, data3, data4, data5, data6, data7, data8, data9, obj):
